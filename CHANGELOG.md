...
- Multica REST API **v0.4.36** compatibility.
- `multica_create_task` supports `status`, `label_ids`, `start_date`, and `due_date`.
- `multica_update_task` supports `position`, `start_date`, `due_date`, `clear_start_date`, `clear_due_date`, `parent_issue_id`, `detach_parent`, and `project_id`.
- Task responses include `status_category`, `status_name`, `revision`, `last_activity_at`, `properties`, and `labels`.
- Project responses include `start_date`, `due_date`, and `resource_count`.
- Agent responses include `runtime_bound`, `permission_mode`, `thinking_level`, and custom-env metadata flags.

### Changed

- Multica REST API target bumped from **v0.3.31** to **v0.4.36**.
- Custom workspace issue statuses are passed through to the API instead of being rejected client-side.
