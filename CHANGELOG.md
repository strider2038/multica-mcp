...
- Multica REST API **v0.4.11** compatibility.
- `multica_create_task` supports `status`, `label_ids`, `start_date`, and `due_date`.
- `multica_update_task` supports `position`, `start_date`, `due_date`, `clear_start_date`, `clear_due_date`, `parent_issue_id`, `clear_parent`, `project_id`, and `clear_project`.
- Task responses include `labels` and `properties`; project responses include `start_date`, `due_date`, and `resource_count`; agent responses include `permission_mode` and related metadata.

### Changed

- Multica REST API target bumped from **v0.3.31** to **v0.4.11**.
